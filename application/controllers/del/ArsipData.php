...
class ArsipData extends CI_Controller {
	...
	public function index(){
        ...
        // $user = $this->arsip->getUser($this->session->userdata('token'))->row_array();
		...
		$id_menu = "182";
		$level_access_pass = $this->arsip->getLevel($group_user, $id_menu)->result();
		...
		$jumlahData = $this->arsip->dokumenAktif($group_user, $department, $idJudul, $ket, $det, $start, $end, $group, $cost_center, 0, 0)->num_rows();
		...
		$data['dokumenAktif'] = $this->arsip->dokumenAktif($group_user, $department, $idJudul, $ket, $det, $start, $end, $group, $cost_center, $rpp, $page)->result();
		...
		$data['listDept'] = $this->arsip->getDept()->result();
		...
		$this->load->view('page_view/arsipData');
	...
	function getDataLog(){
		...
		foreach($logData as $ld){
			...
			$result .= "' type='button' data-toggle='collapse' data-target='#collapse_" . $idx . "' aria-expanded='true' aria-controls='collapse_" . $idx . "'>";
			...
			if($ld->status !== "VALID"){
				$result .= "<button class='btn btn-outline-success float-right col-lg-2' data-toggle='modal' data-target='#modal_update_step'"; 
			...
			$result .= 			"' aria-labelledby='heading_" . $idx . "' data-parent='#accordion_log'>";
	...
	function getDataFilterJudul(){
		...
		$jumlahData = $this->arsip->dokumenAktif($group_user, $department, $id_judul, $ket, $det, $start, $end, $group, $cost_center, 0, 0)->num_rows();

		$jumlahPaging = ceil($jumlahData / $rpp);
		$halaman = $hn;

		$result = $this->arsip->dokumenAktif($group_user, $department, $id_judul, $ket, $det, $start, $end, $group, $cost_center, $rpp, $hn)->result();
	...
	function test(){
		...
		$folder = "assets/tracking_doc/foto_progress/if_14";
		rmdir($folder);
	...
	function renderData(){
		...
		$result = $this->arsip->dokumenAktif($group_user, $department, $id_judul, $ket, $det, $start, $end, $group, $cost_center, $rpp, $page)->result();
	...
	function infoDoku(){
		...
		$cost_center = $user_2['cost_center'];
