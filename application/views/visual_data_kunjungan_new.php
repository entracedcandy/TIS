...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // AMBIL DATA RAW SEKALI SAJA DARI CONTROLLER
    ...
    // --- TOGGLE FILTER FORM (RANGE VS QUARTER) ---
    ...
    function updateLogFilter() {
        ...
        // Atur visibilitas tombol reset log
        ...
        // Filter dari array, sangat ringan!
        let filteredData = rawData.filter(visit => {
            ...
            if (activeKategori) {
                ...
                if (activeKategori === 'Layer') {
                    matchKategori = (visit.kategori_visit === 'Layer');
                } else if (activeKategori === 'Agen/Subagen/Lainnya') {
                    matchKategori = ['Agen', 'Sub Agen', 'Kantor'].includes(visit.kategori_visit);
                } else if (activeKategori === 'Others') {
                    matchKategori = !nonOtherVisits.includes(visit.kategori_visit);
                } else {
    ...
    // ==========================================
    // FUNGSI UTAMA 2: RENDER HTML LOG
    // ==========================================
    function renderTableData(data) {
            ...
            logTableBody.innerHTML = `
                <tr>
                    <td colspan="13" class="text-center py-5 text-muted">
                        ...
                        Tidak ada data log yang cocok dengan kriteria filter saat ini.
                    </td>
                </tr>`;
            return;
        }

        // Limit render ke 200 agar browser tidak freeze
        ...
        dataToRender.forEach(visit => {
            htmlContent += `
                <tr class="visit-row" 
                    ...
                    data-vip="${escapeHTML(visit.vip_farm)}" 
                    ...
                    <td class="visit-cell">
                        <small class="text-muted"><i class="far fa-clock mr-1"></i>${formatTanggal(visit.waktu_kunjungan)}</small>
                    </td>
                    ...
                    <td class="text-center visit-cell">
                        <a href="https://www.google.com/maps/search/?api=1&query=${escapeHTML(visit.latitude)},${escapeHTML(visit.longitude)}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill" onclick="event.stopPropagation();">
                            <i class="fas fa-map-marker-alt mr-1"></i>Lihat
                        </a>
                    </td>
                    <td class="visit-cell extra-wide">
                        <small>${escapeHTML(visit.catatan)}</small>
                    </td>
                </tr>`;
        });
        ...
        if (data.length > renderLimit) {
            htmlContent += `
                <tr>
                    <td colspan="13" class="text-center py-3 bg-light text-muted" style="font-weight: 500;">
                        <i class="fas fa-info-circle mr-1"></i> Menampilkan ${renderLimit} dari ${data.length} total data. Gunakan kotak pencarian untuk hasil lebih spesifik.
                    </td>
                </tr>`;
        }
    ...
    // --- EVENT: Filter & Search Trigger ---
    if (vipFilterDropdown) vipFilterDropdown.addEventListener('change', updateLogFilter);
    if (searchInput) {
        searchInput.addEventListener('keyup', function() {
            clearTimeout(this.timer);
            this.timer = setTimeout(() => updateLogFilter(), 300); // Debounce biar gak lag
        });
    }

    // --- FUNGSI EXPANDABLE ROW LOG ---
    if (logTableBody) {
    ...
    // Reset khusus tabel log tanpa reload halaman
    if(resetLogBtn) {
        resetLogBtn.addEventListener('click', function(e) {
            ...
            // Sembunyikan semua tombol reset parsial
    ...
    // --- HELPER: Update Tabel Surveyor via AJAX ---
    ...
    // --- HELPER: Update Tabel Komposisi via AJAX ---
    function updateCompositionTable(data) {
        if (!compositionTableBody) return;
        compositionTableBody.innerHTML = ''; 
        
        if (!data || data.length === 0) {
             compositionTableBody.innerHTML = `<tr><td colspan="2" class="text-center py-5 text-muted"><i class="fas fa-inbox fa-3x mb-3 d-block" style="opacity: 0.3;"></i>Tidak ada data komposisi.</td></tr>`;
             return;
        }
        ...
        data.forEach(row => {
            ...
            const tr = `
                <tr class="composition-row" data-kategori="${escapeHTML(row.kategori)}" style="cursor: pointer; transition: all 0.3s ease;">
                    ...
                    <td class="text-right align-middle"><strong style="color: ${color}; font-size: 16px;">${percent}%</strong></td>
                </tr>
            `;
    ...
    // Logic RBAC Tombol Reset Dashboard
    if(resetSurveyorBtn) resetSurveyorBtn.style.display = 'none';
    if(resetCompBtn) resetCompBtn.style.display = 'none';
    if(resetLogBtn) resetLogBtn.style.display = 'none';
