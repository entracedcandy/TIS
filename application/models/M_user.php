...
class M_user extends CI_Model
{
  ...
  {
    $data['user'] = $this->premix->get_where('tb_user', ['email' =>
    ...
    if ($uploadImage) {
      ...
      if ($this->upload->do_upload('image')) {
        ...
        $this->premix->set('image', $gambarBaru);
        echo $gambarBaru;
      } else {
    ...
    // QUERY UPDATE
    $this->premix->set('nama', $nama);
    $this->premix->where('email', $email);
    $this->premix->update('tb_user');
  ...
  public function changePassword()
  {
    ...
    if (!password_verify($password, $data['user']['password'])) {
      ...
      if ($password == $passwordBaru) {
        ...
        $this->premix->set('password', $password_hash);
        $this->premix->where('email', $this->session->userdata('email'));
        $this->premix->update('tb_user');
  ...
  public function daftarUser()
  {
    ...
    $this->premix->order_by('id_user', 'DESC');
  ...
  public function addUser()
  {
    ...
    // QUERY INSERT DATA
    $this->premix->insert('tb_user', $data);
  ...
  { // QUERY UPDATE
    $this->premix->set('is_active', $user['is_active']);
    $this->premix->set('role_id', $user['role_id']);
    $this->premix->where('id_user', $user['id_user']);
    $this->premix->update('tb_user', $user);
  ...
  {
    $this->premix->where('id_user', $user['id_user']);
    $this->premix->delete('tb_user', $user); // FLASH DATA
