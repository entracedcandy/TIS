...
class M_auth extends CI_Model
{
  ...
  public function login()
  {
    ...
    $user = $this->premix->get_where('tb_user', ['email' => $email])->row_array();
  ...
  public function register()
  {
    ...
    $this->premix->insert('tb_user_token', $user_token);
  ...
  //   if ($user) {
  //     $user_token = $this->premix->get_where('tb_user_token', ['token' => $token])->row_array();

  //     if ($user_token) {
  //       if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
  //         $this->premix->set('is_active', 1);
  //         $this->premix->where('email', $email);
  //         $this->premix->update('tb_user');

  //         $this->premix->delete('tb_user_token', ['email' => $email]);

  //         $this->session->set_flashdata('success', 'Selamat , akun anda berhasil diregistrasi');
  //         redirect('auth');
  //       } else {
  //         $this->premix->delete('tb_user', ['email' => $email]);
  //         $this->premix->delete('tb_user_token', ['email' => $email]);
  ...
  {
    $email = $this->input->post('email');
    $user = $this->premix->get_where('tb_user', ['email' => $email, 'is_active' => 1])->row_array();
  ...
  public function changePassword()
  {
    ...
    $this->premix->set('password', $password);
    $this->premix->where('email', $email);
    $this->premix->update('tb_user');
